...
import json, ntpath, requests, random
...
class Talk(object):
    ...
    @loggedIn
    def sendMentionV3(self, msg_id,to, text="", mids=[], isUnicode=True):
        arrData = ""
        arr = []
        mention = "@kevinxmayu "
        if mids == []:
    ...
    def sendTemplate(self,to,data):
        ...
        itu = LiffContext(chat=anu)
        lol = LiffViewRequest('1602687308-GXq4Vvk9', itu)
        tes = self.issueLiffView(lol)
    ...
    @loggedIn
    def generateFooterTemplate(self,title=None,link=None,iconlink=None):
        userTicket = self.generateUserTicket()
        title = title if title else "Selfbot version"
        link = link if link else 'line://ti/p/'+userTicket
        iconlink = iconlink if iconlink else 'https://obs.line-apps.com/os/p/%s' % self.profile.mid
        data = {"label":title,"iconUrl":iconlink,"linkUrl":link}
        return data

    @loggedIn
    def sendMessageWithFooterTemplate(self,to,text,title=None,link=None,iconlink=None,isHelper=False):
        if isHelper:
            footer = self.generateFooterTemplate(title=title,link="line://ti/p/~notyourpriority",iconlink=iconlink)
        else:
    ...
    @loggedIn
    def sendImageWithFooterTemplate(self,to,ocu,piu=None,title=None,link=None,iconlink=None):
        if piu == None:
            piu = ocu
        footer = self.generateFooterTemplate(title,link,iconlink)
        data = {
    ...
    @loggedIn
    def sendImageWithTemplate(self,to,ocu,piu=None):
        if piu == None:
            piu = ocu
        data = {
            "messages": [{
    ...
    @loggedIn
    def sendVideoWithTemplate(self,to,ocu,piu):
        data = {
            "messages": [{
                "type": "video",
                "originalContentUrl": ocu,
    ...
    @loggedIn
    def sendAudioWithTemplate(self,to,ocu):
        data = {
            "messages": [{
                "tyoe": "audio",
                "originalContentUrl": ocu,
